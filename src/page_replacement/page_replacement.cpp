    ...
    for (int i = 0; i < pages.size(); ++i) {
        ...
        if (find(memory.begin(), memory.end(), page) == memory.end()) {
            if (memory.size() < frames) {
                ...
                for (int j : memory) {
                    auto it = recent.find(j);
                    if (it != recent.end() && it->second < lru_index) {
                        lru_page = j;
                        lru_index = it->second;
                    }
                }
    ...
    for (int i = 0; i < pages.size(); ++i) {
        ...
        if (find(memory.begin(), memory.end(), page) == memory.end()) {
            if (memory.size() < frames) {
                ...
                int index_to_replace = 0, farthest = -1;
                for (int j = 0; j < (int)memory.size(); ++j) {
                    int next_use = pages.size();
                    for (int k = i + 1; k < (int)pages.size(); ++k) {
                        if (pages[k] == memory[j]) { next_use = k; break; }
                    }
                    if (next_use > farthest) { farthest = next_use; index_to_replace = j; }
                }
...
void runPageReplacement() {
    ...
    if (frameCount <= 0 || refSize <= 0) {
        cout << "Invalid input. Frames and reference size must be > 0.\n";
        return;
    }
